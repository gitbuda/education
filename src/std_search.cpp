...
std::string random_string(std::string::size_type len) {
  static auto &chars = "abcd";
  thread_local static std::mt19937 rg{std::random_device{}()};
  thread_local static std::uniform_int_distribution<std::string::size_type>
      ridx(0, sizeof(chars) - 2);
  std::string s;
  s.reserve(len);
...
int main() {
  ...
  auto str_random = random_string(1000);
  auto part_random = random_string(4);
  it = std::search(str_random.begin(), str_random.end(), part_random.begin(),
                   part_random.end());
  std::cout << str_random << " " << part_random << std::endl;
  std::cout << it - str_random.begin() << std::endl;

  // TODO: do the benchmark with std::boyer_moore_searcher
