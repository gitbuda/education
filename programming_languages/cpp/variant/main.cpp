...
struct Struct {
  ...
  Struct(const Struct &) { println("Struct copy constructor called."); }
  Struct(Struct &&) { println("Struct move constructor called."); }
...
int main() {
  ...
  try {
    ...
    println("Error during variant emplace operation");
  }
  ...
  std::vector<std::variant<int, long, double, std::string>> vec = {10, 15l, 1.5,
                                                                   "hello"};
    ...
    std::visit(
        overloaded{
            ...
            [](const std::string &arg) {
              print<0>("std::string: ", std::quoted(arg));
            },
        },
        v);
  }
